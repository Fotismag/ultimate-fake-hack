# 💻 Ultimate Fake Terminal Hack Prank
...
* **⏱️ Sudo Cache Awareness:** Automatically detects if `sudo` credentials are cached. If you authenticated recently on a real system command, it skips the password prompt entirely to maintain absolute tactical realism.
* **🛑 Standard Root Enforcement:** Trying to run `hack` without `sudo` instantly yields a believable permission error.
* **🟢 Matrix Effects:** Floods the system window with rolling binary streams, delay counters, and hex verification keys.
